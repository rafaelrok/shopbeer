...
public class SalesImpl implements SalesQuery {
	...
	@Override
	public Page<Sale> filtered(SaleFilter filter, Pageable pageable) {
	...
	@Transactional(readOnly = true)
	@Override
	public Sale searchWithItems(Long code) {
		CriteriaBuilder builder = manager.getCriteriaBuilder();
		CriteriaQuery<Sale> query = builder.createQuery(Sale.class);
		Root<Sale> root = query.from(Sale.class);
	...
	public List<SalesMonthDTO> totalByMonth() {
		...
		for (int i = 1; i <= 6; i++) {
			...
			boolean hasMonth = salesMonth.stream().filter(v -> v.getMonth().equals(idealMonth)).findAny().isPresent();
			if (!hasMonth) {
	...
	public List<OriginSalesDTO> totalByOrigin() {
		...
		for (int i = 1; i <= 6; i++) {
			...
			boolean hasMonth = salesNationality.stream().filter(v -> v.getMonth().equals(idealMonth)).findAny().isPresent();
			if (!hasMonth) {
...
//		criteria.createAlias("costumer", "c");
//
//		if (filter != null) {
...
//
//			if (filter.getStatusSale() != null) {
//				criteria.add(Restrictions.eq("status", filter.getStatusSale()));
//			}
//
//			if (filter.getSince() != null) {
//				LocalDateTime desde = LocalDateTime.of(filter.getSince(), LocalTime.of(0, 0));
//				criteria.add(Restrictions.ge("createDate", desde));
//			}
//
//			if (filter.getUntil() != null) {
//				LocalDateTime ate = LocalDateTime.of(filter.getUntil(), LocalTime.of(23, 59));
//				criteria.add(Restrictions.le("createDate", ate));
//			}
//
//			if (filter.getMinimumValue() != null) {
//				criteria.add(Restrictions.ge("amount", filter.getMinimumValue()));
//			}
//
//			if (filter.getMaximumValue() != null) {
//				criteria.add(Restrictions.le("amount", filter.getMinimumValue()));
//			}
//
//			if (!StringUtils.hasText(filter.getCostumerName())) {
//				criteria.add(Restrictions.ilike("c.name", filter.getCostumerName(), MatchMode.ANYWHERE));
//			}
//
//			if (!StringUtils.hasText(filter.getCpforCnpjCostumer())) {
//				criteria.add(Restrictions.eq("c.cpfOrCnpj", TypePerson.removeformatting(filter.getCpforCnpjCostumer())));
//			}
//		}
//	}
